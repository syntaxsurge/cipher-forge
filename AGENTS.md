...
- Arcade Runtime: External game repositories are cloned under `vendor/arcade/*`; distributable static artifacts are synced into `public/games/pong`, `public/games/snake`, and `public/games/asteroids`; app-side metadata and icon mapping live in `src/features/arcade/registry/arcadeGames.ts`; each game HUD includes pause/resume controls (`P`/`Esc` and touch/click buttons).
