...
- `/arcade/[gameId]` - Embedded game runtime page that launches a specific game from `public/games` with fullscreen controls and keyboard/touch pause guidance.
...
- `/forge/[challengeId]/prove` - Client-rendered challenge workbench that loads published metadata, embeds the selected arcade game (`pong`, `snake`, `asteroids`), prechecks browser ZK runtime assets, performs proving worker warmup, explains the ZK settlement model, supports local victory-code checks, auto-populates challenger/session state from Convex lifecycle data, generates/verifies a Noir UltraHonk proof in-browser, starts on-chain sessions (`create_session`) with challenger-wallet authorization, and submits proof/public-input bytes to the Soroban game contract (`submit_proof`).
...
- Arcade Runtime: External game repositories are cloned under `vendor/arcade/*`; distributable static artifacts are synced into `public/games/pong`, `public/games/snake`, and `public/games/asteroids`; app-side metadata and icon mapping live in `src/features/arcade/registry/arcadeGames.ts`; each game HUD includes pause/resume controls (`P`/`Esc` and touch/click buttons) and uses a shared modal dialog utility (`public/games/shared/game-dialog.js`) for start, pause, resume, and game-over flows.
