...
- `/forge` - Creator console with authenticated draft challenge creation, publish/share actions, and lifecycle tracking.
...
- State/Data: Convex stores challenge records across `draft`, `published`, and `settled` lifecycle states, including arcade game preset metadata, challenge key input rules, commitment hash, and session settlement fields; creator mutations enforce identity with `ctx.auth.getUserIdentity()` while marketplace/leaderboard queries remain public.
